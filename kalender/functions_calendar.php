...
function getDayActivities($userId, $monthNr, $yearNr, $dayNr){
    $result = null;
    $conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);

    $beginDate = $yearNr."-".$monthNr."-01 00:00:00";
    $endDate = $yearNr."-".$monthNr."-31 23:59:59";
    $sql = 'SELECT (SELECT name FROM subjects WHERE id = time_reportings.subject_id) AS subject, 
                                            (SELECT name FROM activities WHERE id = time_reportings.activity_id) AS activity, 
                                            sum(duration), date(report_date) FROM time_reportings WHERE user_id=?
                                            AND report_date between ? and ?
                                            AND weekday(report_date) = ?
                                            GROUP BY time_reportings.subject_id, time_reportings.activity_id
                                            ORDER BY report_date ASC';
    $stmt = $conn -> prepare($sql);


    $stmt->bind_param("issi", $userId,$beginDate, $endDate, $dayNr);
    $stmt -> bind_result($subjectFromDb, $activityFromDb, $durationFromDb, $dateFromDb);
    $stmt -> execute();
    echo $stmt->error;
    $events = array();
